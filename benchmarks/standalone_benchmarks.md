...
`spectral_norm.py` の次に、Python の異なる実行パターンを見るための7本。
各 `.py` にアルゴリズム・入力・計測コードを収めてあるので、そのファイルだけを
別のディレクトリにコピーして実行できる。SQLAlchemy以外の6本はPython 3.10以降の
標準ライブラリだけを使い、外部パッケージ、`pyperf`、データファイル、ネットワーク接続は不要。
`sqlalchemy_declarative.py` はSQLAlchemyを必要とし、比較では両Pythonに同じバージョンと
依存パッケージを用意する。元のpyperformanceと同じSQLAlchemy 1.4.19を使用している。
...
| [sqlalchemy_declarative.py](sqlalchemy_declarative.py) | インメモリSQLiteに100人と100住所を1件ずつcommitして挿入し、全員の読み出しを100回実行 | ORM、属性の計装、ディスクリプタ、Python呼び出し、SQLiteのC処理 |
...
python3 sqlalchemy_declarative.py --loops 1 --warmups 3 --values 10 --json > sqlalchemy.json
```
...
- SQLAlchemy: 元の100人・100住所、各オブジェクトのcommit、全員の読み出し100回を保持。
  DB作成、前回の行の削除、結果検証は計測外。全行の値と人・住所の対応を検証する。
  `--rows`で人数を変更できるが、main比較にはデフォルトの100を使う。
