#!/usr/bin/env python3
"""Render a small scene using Python objects and overloaded operators.

Exercises short-lived objects, method calls, arithmetic special methods,
floating-point arithmetic and recursive shading. Keeps the original
100 x 100 scene and renderer; no image file is written during the run.
The complete RGB buffer is checked after each timing sample.
...
BENCHMARK = "raytrace"
WORKLOAD = "render the original sphere/checkerboard scene at 100 x 100 pixels"
EXPECTED_PIXEL_SHA256 = (
    "11f2c36b02044f115c3937cae1156e9528b95da70f9d6b737a262f06757c2e6b"
)
...
class Scene(object):
    ...
    def _lightIsVisible(self, lightPoint, p):
        for o, s in self.objects:
            t = o.intersectionTime(Ray(p, lightPoint - p))
    ...
    def visibleLights(self, p):
        result = []
        for lightPoint in self.lightPoints:
            if self._lightIsVisible(lightPoint, p):
                result.append(lightPoint)
        return result
...
def check_result(canvas):
    if (canvas.width, canvas.height) != (DEFAULT_WIDTH, DEFAULT_HEIGHT):
        raise RuntimeError("Raytracer returned invalid image dimensions")
    digest = hashlib.sha256(canvas.bytes.tobytes()).hexdigest()
    if digest != EXPECTED_PIXEL_SHA256:
        raise RuntimeError(f"Raytracer returned invalid pixels: {digest}")
    return {"pixels": canvas.width * canvas.height, "pixel_sha256": digest}
