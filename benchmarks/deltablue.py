#!/usr/bin/env python3
"""Propagate changes through a pure-Python constraint graph.

Exercises polymorphic method calls, inheritance/super, attribute access
and mutable object graphs. This is a synthetic model of incremental
constraint solving, rather than an end-to-end application. Original
algorithms and size are retained; reported failures now raise errors.
...
BENCHMARK = "deltablue"
SIZE = 100
WORKLOAD = "100-link equality chain and 100 scale/offset constraints"
# The last pair stays backward after editing dst to 1050: final src is
# (1050 - 2000) / 5 == -190. The other pairs propagate forward.
EXPECTED_DESTINATIONS = tuple(i * 5 + 2000 for i in range(SIZE - 1)) + (1050,)
...
def projection_test(n):
    ...
    return src.value, tuple(dest.value for dest in dests)
...
def delta_blue(n):
    chain_last = chain_test(n)
    source_last, destinations = projection_test(n)
    return chain_last, source_last, destinations


def benchmark():
    return delta_blue(SIZE)


def check_result(result):
    chain_last, source_last, destinations = result
    if chain_last != 99 or source_last != -190 or destinations != EXPECTED_DESTINATIONS:
        raise RuntimeError(f"DeltaBlue constraint values are invalid: {result!r}")
    return {
        "chain_last": chain_last,
        "source_last": source_last,
        "projection_sum": sum(destinations),
    }
