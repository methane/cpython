#!/usr/bin/env python3
"""Train and run a small pure-Python byte-pair tokeniser.

Exercises bytes/dict keys, Counter updates, tuple hashing, list slicing,
nested loops and comprehensions. The original 31 KB corpus is embedded;
training and encoding are both timed, including regex word splitting.
...
VOCAB_SIZE = 1024
EXPECTED_TOKEN_COUNT = 10335
EXPECTED_TOKEN_SHA256 = (
    "ae6aed60fc44c7a735e5d9e9f896ece86ad7209ab658e0599c332534c604242e"
)
PATTERN = r"'s|'t|'re|'ve|'m|'ll|'d| ?[a-zA-Z]+| ?\d+| ?[^\sa-zA-Z\d]+|\s+(?!\S)|\s+"
WORKLOAD = "train a 1024-token vocabulary and encode the Frankenstein introduction"
...
# Exact input from bm_bpe_tokeniser/data/frankenstein_intro.txt (Mary Shelley).
# Embedded verbatim so copying this file is sufficient to run the benchmark.
DATA = """Letter 1
...
def benchmark():
    enc = SimpleBytePairEncoding.train(DATA, vocab_size=VOCAB_SIZE, pat_str=PATTERN)
    tokens = enc.encode("hello world")
    if enc.decode(tokens) != "hello world":
        raise RuntimeError("BPE smoke-test round trip failed")
    return enc, enc.encode(DATA)


def check_result(result):
    enc, tokens = result
    if len(enc.mergeable_ranks) != VOCAB_SIZE or enc.decode(tokens) != DATA:
        raise RuntimeError("BPE vocabulary size or full-text round trip is invalid")
    digest = hashlib.sha256(
        b"".join(token.to_bytes(2, "little") for token in tokens)
    ).hexdigest()
    if len(tokens) != EXPECTED_TOKEN_COUNT or digest != EXPECTED_TOKEN_SHA256:
        raise RuntimeError(f"BPE token stream is invalid: {len(tokens)=}, {digest=}")
    return {"tokens": len(tokens), "token_sha256": digest}
