...
// BEGIN BYTECODES //
    ...
    op(_GUARD_TYPE_VERSION_LOCKED, (type_version/2, owner -- owner)) {
        ...
        if (sym_matches_type_version(owner, type_version)) {
            ...
            PyTypeObject *probable_type = _PyType_LookupByVersion(type_version);
    ...
    op(_BINARY_OP, (lhs, rhs -- res, l, r)) {
        ...
        // guards. Hints derived from static values still need runtime
        // checks before they become facts. Applied only to ops where
    ...
    op(_BINARY_OP_SUBSCR_CHECK_FUNC, (container, unused -- container, unused, getitem)) {
        ...
        if (tp != NULL) {
            ...
            sym_set_probable_value(getitem, getitem_o);
        }
    }

    op(_METHOD_CHECK_PERIODIC, (--)) {
    }

    op(_METHOD_SUBSCR_CHECK_FUNC, (type_version/2, func_version/2, container, unused -- container, unused, getitem)) {
        (void)type_version;
        (void)func_version;
        getitem = sym_new_type(ctx, &PyFunction_Type);
    }
    ...
    op(_IS_GLOBAL_BINDING, (ptr/4, left -- b, l)) {
        (void)ptr;
    ...
    op(_LOAD_GLOBAL_BINDING, (ptr/4 -- value)) {
        (void)ptr;
        // An object can change __class__ without invalidating its binding.
    ...
    op(_LOAD_ATTR_MODULE_CONST, (dict/4, value/4, owner -- attr, o)) {
        (void)dict;
        // Retain the atomic guard/load, including its runtime module check.
        attr = PyModule_Check((PyObject *)value)
            ? sym_new_not_null(ctx) : sym_new_const(ctx, (PyObject *)value);
        o = owner;
    }
    ...
    op(_GUARD_LOAD_SUPER_ATTR_METHOD, (global_super_st, class_st, unused -- global_super_st, class_st, unused)) {
        ...
        PyObject *known = sym_get_const(ctx, class_st);
        if (known != NULL && !PyType_Check(known)) {
            ctx->contradiction = true;
            ctx->done = true;
        }
        else if (known == NULL) {
            /* The guard accepts subclasses of type. Neither an exact
             * metaclass nor a constant class follows from this check. */
            sym_set_non_null(class_st);
        }
    }
    ...
    op(_INIT_CALL_BOUND_METHOD_EXACT_ARGS, (callable, self_or_null, unused[oparg] -- callable, self_or_null, unused[oparg])) {
        ...
        if (bound_method != NULL && Py_TYPE(bound_method) == &PyMethod_Type) {
            ...
            sym_set_probable_value(callable, method->im_func);
            self_or_null = sym_new_not_null(ctx);
            sym_set_probable_value(self_or_null, method->im_self);
        }
        else {
            callable = sym_new_not_null(ctx);
            self_or_null = sym_new_not_null(ctx);
    ...
    op(_GUARD_TYPE_VERSION_FAMILY, (versions01/4, versions23/4, owner -- owner)) {
        (void)versions01;
        (void)versions23;
        /* A disjunction of versions does not prove one exact type. */
        sym_set_non_null(owner);
    }

    op(_LOAD_ATTR_METHOD_DYNAMIC, (owner -- attr, self)) {
        attr = sym_new_type(ctx, &PyFunction_Type);
        self = owner;
    }

    op(_CHECK_PY_FUNCTION, (callable, unused, unused[oparg] -- callable, unused, unused[oparg])) {
        sym_set_type(callable, &PyFunction_Type);
    }

    op(_CALL_BOOL_ATTRIBUTES, (layout/4, callable, self_or_null, args[oparg] -- res)) {
        (void)layout;
        res = sym_new_type(ctx, &PyBool_Type);
    }
    ...
    op(_CHECK_FUNCTION_VERSION, (func_version/2, callable, self_or_null, unused[oparg] -- callable, self_or_null, unused[oparg])) {
        ...
        else if (!PyFunction_Check(func) || ((PyFunctionObject *)func)->func_version != func_version) {
            ctx->contradiction = true;
            ctx->done = true;
            break;
        }
    ...
    op(_RETURN_VALUE, (retval -- res)) {
        /* Frame transitions are method CFG boundaries, not recorded paths. */
        res = retval;
        ctx->done = true;
    }

    op(_METHOD_RETURN_VALUE, (retval -- res)) {
        res = retval;
        ctx->done = true;
    }
    ...
    op(_FOR_ITER_TIER_TWO, (iter, null_or_index -- iter, null_or_index, next)) {
        ...
        if (type != NULL && type != &PyGen_Type && type->tp_iternext != NULL
            ...
            if (!definite) {
                ...
                int32_t orig_target = this_instr->target;
    ...
    op(_ITER_NEXT_RANGE_COMPACT, (iter, null_or_index -- iter, null_or_index, next)) {
        next = sym_new_compact_int(ctx);
    }
    ...
    op(_CALL_ISINSTANCE_DEFAULT, (metaclass_version/2, callable, null, instance, cls -- res)) {
        (void)metaclass_version;
        res = sym_new_type(ctx, &PyBool_Type);
    }
    ...
    op(_LOAD_SPECIAL, (method_and_self[2] -- method_and_self[2])) {
        ...
        if (type != NULL) {
            ...
            if (descr != NULL && (Py_TYPE(descr)->tp_flags & Py_TPFLAGS_METHOD_DESCRIPTOR)) {
                /* LOAD_SPECIAL expands to _INSERT_NULL + _LOAD_SPECIAL.
                 * Insert _GUARD_TYPE_VERSION before the
    ...
    op(_BINARY_OP_SUBSCR_LIST_INT, (list_st, sub_st -- res, ls, ss)) {
        ...
        ls = list_st;
        ss = sub_st;
        PyObject *index = sym_get_const(ctx, sub_st);
        if (index != NULL && PyLong_CheckExact(index) &&
            _PyLong_IsNonNegativeCompact((PyLongObject *)index))
        {
            uint32_t value = ((PyLongObject *)index)->long_value.ob_digit[0];
            REPLACE_OP(this_instr, _BINARY_OP_SUBSCR_LIST_CONST, 0, value);
        }
    }
