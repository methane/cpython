...
|    KNOWN_CLASS--+       |    |      |
|    |  |  |              | PREDICATE PROBABLE_VALUE(known type)
|    |  | INT*            |    |      |
...
Probable values are static hints; guards are required to make them facts.
...
_PyUOpSymPrint(JitOptRef ref)
{
    ...
    switch (tag) {
        ...
        case JIT_SYM_PREDICATE_TAG:
            ...
            break;
        case JIT_SYM_PROBABLE_VALUE_TAG:
            printf("<probable value %p>", sym->probable_value.value);
            break;
        default:
...
_Py_uop_sym_set_type(JitOptContext *ctx, JitOptRef ref, PyTypeObject *typ)
{
    ...
    switch(tag) {
        ...
        case JIT_SYM_BOTTOM_TAG:
            return;
        case JIT_SYM_PROBABLE_VALUE_TAG:
            if (Py_TYPE(sym->probable_value.value) == typ) {
                sym->probable_value.known_type = true;
            }
            else {
                sym_set_bottom(ctx, sym);
            }
...
_Py_uop_sym_set_type_version(JitOptContext *ctx, JitOptRef ref, unsigned int version)
{
    ...
    switch(tag) {
        ...
        case JIT_SYM_COMPACT_INT:
            ...
            return true;
        case JIT_SYM_PROBABLE_VALUE_TAG:
            if (Py_TYPE(sym->probable_value.value)->tp_version_tag == version) {
                sym->probable_value.known_type = true;
                sym->tag = JIT_SYM_KNOWN_CLASS_TAG;
                sym->cls.type = Py_TYPE(sym->probable_value.value);
                sym->cls.version = version;
                return true;
            }
            else {
...
_Py_uop_sym_set_const(JitOptContext *ctx, JitOptRef ref, PyObject *const_val)
{
    ...
    switch(tag) {
        ...
        case JIT_SYM_BOTTOM_TAG:
            return;
        case JIT_SYM_PROBABLE_VALUE_TAG:
            /* The given value might contradict the static hint,
             * in which case we could return bottom.
             * Just discard the probable value for now */
        case JIT_SYM_NON_NULL_TAG:
        case JIT_SYM_UNKNOWN_TAG:
            make_const(sym, const_val);
            return;
...
_Py_uop_sym_get_type(JitOptRef ref)
{
    ...
    switch(tag) {
        ...
        case JIT_SYM_UNKNOWN_TAG:
            return NULL;
        case JIT_SYM_PROBABLE_VALUE_TAG:
            if (sym->probable_value.known_type) {
                return Py_TYPE(sym->probable_value.value);
            }
            return NULL;
        case JIT_SYM_KNOWN_CLASS_TAG:
            return sym->cls.type;
...
_Py_uop_sym_get_probable_value(JitOptRef ref)
{
    ...
    switch(tag) {
        ...
        case JIT_SYM_KNOWN_CLASS_TAG:
            return NULL;
        case JIT_SYM_PROBABLE_VALUE_TAG:
            return sym->probable_value.value;
        case JIT_SYM_KNOWN_VALUE_TAG:
            return sym->value.value;
    }
    Py_UNREACHABLE();
...
void
_Py_uop_sym_set_probable_value(JitOptContext *ctx, JitOptRef ref, PyObject *value)
{
    // A static lookup may not have produced a usable value.
    if (value == NULL) {
        return;
    }
    JitOptSymbol *sym = PyJitRef_Unwrap(ref);
    ...
    switch(tag) {
        ...
        case JIT_SYM_NON_NULL_TAG:
        case JIT_SYM_UNKNOWN_TAG:
            sym->tag = JIT_SYM_PROBABLE_VALUE_TAG;
            sym->probable_value.known_type = false;
            sym->probable_value.value = value;
            return;
        case JIT_SYM_PROBABLE_VALUE_TAG:
            if (sym->probable_value.value != value) {
                sym_set_bottom(ctx, sym);
            }
            return;
        case JIT_SYM_KNOWN_CLASS_TAG:
    ...
    int arg_len)
{
    ...
    frame->builtins_identity_checked = false;
    frame->func = NULL;
    frame->caller = false;
...
_Py_uop_symbols_test(PyObject *Py_UNUSED(self), PyObject *Py_UNUSED(ignored))
{
    ...
    // Test probable values

    /* Test that probable values aren't treated as known values*/
    JitOptRef rv1 = _Py_uop_sym_new_unknown(ctx);
    _Py_uop_sym_set_probable_value(ctx, rv1, val_42);
    TEST_PREDICATE(!_Py_uop_sym_matches_type(rv1, &PyLong_Type), "probable value is treated as known");
    TEST_PREDICATE(_Py_uop_sym_get_const(ctx, rv1) == NULL, "probable value is treated as known");
    TEST_PREDICATE(!_Py_uop_sym_is_compact_int(rv1), "probable value is treated as known");

    /* Test that setting type or value narrows correctly */
    JitOptRef rv2 = _Py_uop_sym_new_unknown(ctx);
    _Py_uop_sym_set_probable_value(ctx, rv2, val_42);
    _Py_uop_sym_set_const(ctx, rv2, val_42);
    TEST_PREDICATE(_Py_uop_sym_matches_type(rv2, &PyLong_Type), "probable value doesn't narrow");
    TEST_PREDICATE(_Py_uop_sym_get_const(ctx, rv2) == val_42, "probable value doesn't narrow");

    JitOptRef rv3 = _Py_uop_sym_new_unknown(ctx);
    _Py_uop_sym_set_probable_value(ctx, rv3, val_42);
    _Py_uop_sym_set_type(ctx, rv3, &PyLong_Type);
    TEST_PREDICATE(_Py_uop_sym_matches_type(rv3, &PyLong_Type), "probable value doesn't narrow");
    TEST_PREDICATE(_Py_uop_sym_get_const(ctx, rv3) == NULL, "probable value with type is treated as known");

    JitOptRef rv4 = _Py_uop_sym_new_unknown(ctx);
    _Py_uop_sym_set_probable_value(ctx, rv4, val_42);
    _Py_uop_sym_set_type_version(ctx, rv4, PyLong_Type.tp_version_tag);
    TEST_PREDICATE(_Py_uop_sym_matches_type(rv4, &PyLong_Type), "probable value doesn't narrow");
    TEST_PREDICATE(_Py_uop_sym_get_const(ctx, rv4) == NULL, "probable value with type is treated as known");

    /* Test contradictions */
    _Py_uop_sym_set_type(ctx, rv1, &PyFloat_Type);
     TEST_PREDICATE(_Py_uop_sym_is_bottom(rv1), "probable value cast to other type isn't bottom");
    _Py_uop_sym_set_type_version(ctx, rv2, PyFloat_Type.tp_version_tag);
     TEST_PREDICATE(_Py_uop_sym_is_bottom(rv2), "probable value cast to other type version isn't bottom");
