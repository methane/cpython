...
class Optimizer:
    ...
    def _find_live_blocks(self) -> set[_Block]:
        live: set[_Block] = set()
        # Preserve data and assembler directives before the first entry point,
        # too. Constant-pool labels are referenced by loads, not branches.
        todo: set[_Block] = {self._root}
        todo.update(b for b in self._blocks() if b.label in self.globals)
        while todo:
            block = todo.pop()
            live.add(block)
