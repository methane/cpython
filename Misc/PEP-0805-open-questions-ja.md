...
アクセスする可能性があります。現在の通常ビルドは ThreadGroup ごとに
実行を直列化し、外国グループからの参照カウントの合算を所有グループの
キューへ送ります。しかし、合算済みの LOCAL オブジェクトを B で解放する
経路や、A の全スレッド終了後の処理は未解決です。GC の finalizer や
`tp_clear` にも同じ実行場所の問題があります。

通常起動でグループ間の並列実行を有効にした後、この問題に関連する
回収待ちが既存テストでも再現しました。ネイティブの VM 検証が作る LOCAL
なクラスの weakref が、所有グループの全スレッド終了後に BRC キューへ
残ります。何もしない Python 関数を新しいグループで実行する検証を30回
繰り返すと、10回 GC を行った後でも90個の参照が残ります。キューを調べると、
各グループに参照数1の LOCAL な `weakref.ReferenceType` が3個ありました。
これらは参照先が消滅済みで、コールバックもありません。
Main だけで同じ検証を繰り返す対照ケースでは、この増加はありません。
`test_vm_heap_loads` などの既存テストも参照リーク検証に失敗します。
ログは `/tmp/pep805-base/departed-weakref-probe.log` と
`test-default-startup-ownership-refleak.log` です。
