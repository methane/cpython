...
class TestMethodDispatchGeneration(unittest.TestCase):
    def test_single_dispatch_table_and_unused_recording_opcode(self):
        import io
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with open(filename, "w") as file:
                file.write(parser.BEGIN_MARKER + source + parser.END_MARKER)
            analysis = analyze_files([filename])
        self.assertNotIn("TRACE_RECORD", analysis.opmap)
        self.assertNotIn(255, analysis.opmap.values())
        output = io.StringIO()
        writer = CWriter(output, 0, False)
        target_generator.write_opcode_targets(analysis, writer)
        target_generator.write_tailcall_dispatch_table(analysis, writer)
        generated = output.getvalue()
        self.assertNotIn("tracing", generated)
        self.assertNotIn("TRACE_RECORD", generated)
        self.assertIn("&&TARGET_NOP", generated)
        self.assertIn("[NOP] = _TAIL_CALL_NOP", generated)
        self.assertIn("[255] = _TAIL_CALL_UNKNOWN_OPCODE", generated)
