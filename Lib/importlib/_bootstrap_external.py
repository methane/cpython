...
_pathseps_with_colon = frozenset(f':{s}' for s in path_separators)
...
SOURCE_SUFFIXES = SynchronizedList(['.py'])
if _MS_WINDOWS:
    SOURCE_SUFFIXES.append('.pyw')

EXTENSION_SUFFIXES = SynchronizedList(_imp.extension_suffixes())

BYTECODE_SUFFIXES = SynchronizedList(['.pyc'])
# Deprecated.
...
def _get_supported_file_loaders():
    ...
    if hasattr(_imp, 'create_dynamic'):
        ...
        extension_loaders.append((ExtensionFileLoader,
                                  SynchronizedList(_imp.extension_suffixes())))
...
def _set_bootstrap_module(_bootstrap_module):
    ...
    if type(module.__dict__) is not SynchronizedDict:
        module.synchronize()
