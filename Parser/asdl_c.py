...
ast_dealloc(PyObject *op)
{
    ...
    Py_CLEAR(self->dict);
    /* Deallocation reads native type metadata, even when the type belongs
       to a different ThreadGroup. It does not publish a type reference. */
    freefunc free_func = tp->tp_free;
...
int PyAST_Check(PyObject* obj)
{
    ...
    /* The canonical AST type is private interpreter metadata. A native
       subtype test does not publish it, even when another group created it.
       Preserve the checked __class__ lookup on obj, and use ordinary access
       rules if a custom metaclass supplies __instancecheck__. */
    if (PyType_CheckExact(state->AST_type)) {
        return _PyObject_RealIsInstance(obj, state->AST_type);
    }
    if (PyObject_CheckAccess(state->AST_type) == NULL) {
        return -1;
    }
