...
static bool mi_heap_page_collect(mi_heap_t* heap, mi_page_queue_t* pq, mi_page_t* page, void* arg_collect, void* arg2 ) {
  ...
  if (mi_page_all_free(page)) {
    ...
    if (!freed && collect == MI_ABANDON) {
      // Deferring the free may have moved the page out of the full queue.
      size_t bin = mi_page_is_in_full(page) ? MI_BIN_FULL : _mi_bin(page->block_size);
...
bool mi_heap_visit_blocks(const mi_heap_t* heap, bool visit_blocks, mi_block_visit_fun* visitor, void* arg) {
  ...
  // CPython's GC visits other threads' heaps while the world is stopped.
  // Drain the heap-level list before reporting freed blocks as live objects.
